# chapter5 / example5-2.py
# 목표: 트레일링 스탑 구현
#
# 트레일링 스탑 흐름:
#   매수 체결 감지 (OnReceiveChejanData) → 실시간 현재가 등록 (SetRealReg)
#   → 현재가 수신 (OnReceiveRealData) → 매수 후 최고가 갱신
#   → 최고가 대비 -1.5% 이하 시 시장가 매도 → 감시 목록에서 제거
#
# 스탑로스와 차이점: 매입가 기준 아닌 매수 후 최고가(매입후고가) 기준으로 손절
...
class KiwoomAPI(QMainWindow):
    ...
    def _receive_realdata(self, sJongmokCode, sRealType, sRealData):
        ...
        if sRealType == "주식체결":
            ...
            매입후고가 = self.realtime_watchlist_df.at[sJongmokCode, "매입후고가"]
            매입후고가 = max(현재가, 매입후고가) if 매입후고가 else 현재가
            self.realtime_watchlist_df.at[sJongmokCode, "매입후고가"] = 매입후고가
    ...
    def receive_chejandata(self, sGubun, nItemCnt, sFIdList):
        ...
        if sGubun == "0":
            ...
            if 매매구분 == "매수" and 체결수량 > 0 and 미체결수량 == 0:
                print(f"종목코드: {종목코드} 트레일링 스탑로스 감시 편입!")
