...
#   → 주문 후 10초 경과 시 미체결수량만큼 취소 주문 발송
#
...
class KiwoomAPI(QMainWindow):
    ...
    def limit_buy_order(self):
        self.send_order(
            ...
            "5000",           # 화면번호
            self.account_num, # 계좌 번호
            1,                # 주문유형, 1:신규매수, 2:신규매도, 3:매수취소, 4:매도취소, 5:매수정정, 6:매도정정, 11: SOR매수, 12: SOR매도, 13: SOR취소
            "005930",         # 종목코드
            1,                # 주문 수량
            40_000,           # 주문 가격 (현재가보다 낮게 설정 → 미체결 유지 → 자동 취소 테스트용)
    ...
    def limit_sell_order(self):
        self.send_order(
            ...
            "005930",         # 종목코드
            1,                # 주문 수량
            90_000,           # 주문 가격 (현재가보다 높게 설정 → 미체결 유지 → 자동 취소 테스트용)
            "00",             # 거래 구분, 00: 지정가, 03: 시장가, 05: 조건부지정가, 06: 최유리지정가, 07: 최우선지정가 등 (KOAStudio 참조)
            "",               # 주문번호 (정정 주문의 경우 사용, 나머지 공백)
        )
